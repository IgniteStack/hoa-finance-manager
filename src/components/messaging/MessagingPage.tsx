...
export function MessagingPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      const prompt = spark.llmPrompt`You are a helpful HOA community manager assistant. The admin wants to send the following message to the community: "${message}"
...
Please generate a professional, friendly WhatsApp message suitable for sending to the community group. Keep it concise and clear.`
      ...
      const generatedMessage = await spark.llm(prompt, 'gpt-4o-mini')
  ...
  const handleQuickCommand = async (command: string) => {
    ...
    try {
      const prompt = spark.llmPrompt`You are a helpful HOA community manager assistant. Generate a professional WhatsApp message for the following purpose: "${command}"
